...
$query = mysqli_query($koneksi, "SELECT * FROM ekstrakurikuler WHERE id='$id'");
$data = mysqli_fetch_array($query);
if (!$data) { echo "Ekstrakurikuler tidak ditemukan."; exit; }
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title><?php echo $data['nama_ekskul']; ?> - SMAN 1 Bengkalis</title>
...
<body>
    ...
    <article class="detail-article">
        <h1><?php echo $data['nama_ekskul']; ?></h1>
        <div class="detail-meta">
            <span><i class="fas fa-users"></i> Ekstrakurikuler</span>
        </div>
        <?php if($data['gambar']) { ?>
            <img src="uploads/<?php echo $data['gambar']; ?>" alt="<?php echo $data['nama_ekskul']; ?>" class="detail-image">
        <?php } ?>
        <div class="detail-content">
            <?php echo nl2br($data['deskripsi']); ?>
        </div>
        <div style="text-align: center; margin-top: 4rem;">
            <a href="ekstrakurikuler.php" class="btn btn-primary"><i class="fas fa-arrow-left"></i> Kembali ke Ekstrakurikuler</a>
        </div>
    </article>
